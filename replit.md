...
- **Two-stage merge guard** in `scripts/post-merge.sh`: stage 1 runs first, stage 2 runs after npm install — both must pass or the merge halts.
  - **Stage 1 — Conflict check**: `scripts/pre-merge-conflict-check.sh` performs a dry-run `git merge --no-commit --no-ff`, scans every affected file for conflict markers (`<<<<<<<`, `=======`, `>>>>>>>`), prints a developer-friendly report of each conflicted file with a first-block excerpt, aborts the dry-run, and exits 1 with `"MERGE HALTED: conflicts detected — developer manual approval required"`. Registered as the `merge-conflict-check` validation so it can be triggered independently before releasing a task. Resolution: check out both branches, fix each conflict block, re-run post-merge.
  - **Stage 2 — Schema drift check**: `scripts/check-schema-drift.sh` (registered as the `schema-drift` validation) fails if the live DB diverges destructively from `shared/schema.ts`. It answers "No, abort" to every drizzle prompt, so it never applies a destructive change; it only flags drops/renames. Run it before any prod release.
- **Merge guard**: `scripts/post-merge.sh` runs both stage 1 (conflict) and stage 2 (schema drift) before applying `db:push --force`, aborting on any conflict or destructive/ambiguous schema change. Note: drizzle's data-loss wording is "delete <x> column" (NOT "drop column") — guards must match that.
