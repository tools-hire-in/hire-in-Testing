...
export default function Approvals() {
  ...
  const { data: items, isLoading } = useQuery<QueueItem[]>({
    queryKey: ["/api/admin/studio/approvals", selectedProjectId],
    queryFn: async () => {
      const url = selectedProjectId
        ? `/api/admin/studio/approvals?projectId=${selectedProjectId}`
    ...
    <AdminLayout>
      <div className="space-y-4">
        ...
        <div className="grid gap-4 lg:grid-cols-[320px_1fr]">
            ...
            {isLoading ? (
              ...
              <Card>
                <CardContent className="flex flex-col items-center gap-2 py-12 text-center text-sm text-muted-foreground">
                  ...
                  <span data-testid="text-empty-approvals">Nothing waiting for marketing.</span>
                </CardContent>
              </Card>
            ) : (
              list.map((a) => (
          ...
          <div>
            {active ? (
              <WorkflowReviewPanel articleId={active} mode="marketing" onDone={() => setSelectedId(null)} />
            ) : (
              <Card>
