...
/**
 * Weekly overtime recognition scan — checks prior Mon-Sun for employees who logged
 * >standard_shift_hours on 3+ distinct working days. Notifies their manager.
 * Deduplication uses the notifications table itself (type='overtime_recognition' + metadata).
 */
export async function runOvertimeRecognitionScan(): Promise<void> {
  try {
    const settings = await getAttendanceSettings();
    const flags = await getFeatureFlags();
    if (!flags.notifications_enabled) return;

    const standardHours = settings.standardShiftHours;

    const nowIST = new Date(Date.now() + 5.5 * 60 * 60 * 1000);
    const dayOfWeek = nowIST.getUTCDay();
    const daysToLastMon = dayOfWeek === 1 ? 7 : (dayOfWeek + 6) % 7 + 7;
    const lastMonday = new Date(nowIST);
    lastMonday.setUTCDate(lastMonday.getUTCDate() - daysToLastMon);
    ...
    console.log(`[overtime-scan] Scanning prior week: ${weekStart} to ${weekEnd} (threshold: ${standardHours}h)`);
    ...
    const results = (await db.execute(sql`
      ...
      WHERE a.date >= ${weekStart}
        AND a.date <= ${weekEnd}
        AND a.status IN ('present', 'late', 'short_day')
        AND CAST(a.total_hours AS NUMERIC) > ${standardHours}
        AND u.is_active = true
        AND u.deleted_at IS NULL
        AND u.attendance_exempt = false
      GROUP BY a.user_id, u.manager_id
      HAVING COUNT(*) >= 3
    `)).rows as any[];

    for (const row of results) {
      const { user_id: empId, manager_id: managerId, days_over: daysOver } = row as any;
      if (!managerId) continue;

      // Dedup via notifications table — no separate table needed
      const alreadyNotified = (await db.execute(sql`
        SELECT id FROM notifications
        WHERE user_id = ${managerId}
          AND type = 'overtime_recognition'
          AND metadata->>'employeeId' = ${empId}
          AND metadata->>'weekStart' = ${weekStart}
        LIMIT 1
      `)).rows as any[];
      if (alreadyNotified.length > 0) continue;

      const empRows = (await db.execute(sql`
        SELECT first_name, last_name FROM admin_users WHERE id = ${empId} LIMIT 1
      `)).rows as any[];
      ...
      await storage.createNotification({
        userId: managerId,
        type: "overtime_recognition",
        title: "Team Member Overtime — Recognise Their Effort",
        message: `${empName} worked over ${standardHours}h on ${daysOver} day(s) last week — a great time to recognise their effort.`,
        isRead: false,
        metadata: {
          employeeId: empId,
          employeeName: empName,
          weekStart,
          weekEnd,
          daysOver: Number(daysOver),
          type: "overtime_recognition",
        },
      }).catch(console.error);

      console.log(`[overtime-scan] Notified manager ${managerId} about ${empName} (${daysOver} days over ${standardHours}h)`);
    }
  } catch (err) {
    console.error("[overtime-scan] runOvertimeRecognitionScan failed:", err);
  }
}
...
export function registerAttendanceExceptionRoutes(app: Express) {
  ...
  // --- Overtime Recognition Alerts (for manager — reads from notifications table) ---
  app.get("/api/attendance/overtime-alerts", requireAuth, requireManagerOrAbove, async (req: Request, res: Response) => {
    try {
      const userId = req.session.userId!;
      const role = req.session.role || "";

      const managerFilter = ["super_admin", "admin", "hr"].includes(role)
        ? sql`n.type = 'overtime_recognition'`
        : sql`n.user_id = ${userId} AND n.type = 'overtime_recognition'`;
      ...
      console.error("[overtime-alerts] list error:", err);
      res.status(500).json({ error: "Failed to fetch overtime alerts" });
    }
  });

  // --- Send Praise ---
  app.post("/api/attendance/overtime-alerts/praise", requireAuth, requireManagerOrAbove, async (req: Request, res: Response) => {
    try {
      const actorId = req.session.userId!;
      const role = req.session.role || "";
      const { employeeId, praiseNote, notificationId } = req.body;

      if (!employeeId || !praiseNote?.trim()) {
        return res.status(400).json({ error: "employeeId and praiseNote are required" });
      }

      // Authorization: non-HR/admin actors must own the originating overtime notification
      // or have the target employee in their team. Prevents IDOR-style praise submissions.
      if (!["super_admin", "admin", "hr"].includes(role)) {
        if (notificationId) {
          // Verify the notification belongs to the actor and references this employee
          const notifRows = (await db.execute(sql`
            SELECT id FROM notifications
            WHERE id = ${notificationId}
              AND user_id = ${actorId}
              AND type = 'overtime_recognition'
              AND metadata->>'employeeId' = ${employeeId}
            LIMIT 1
          `)).rows as any[];
          if (notifRows.length === 0) {
            return res.status(403).json({ error: "You are not authorised to send praise for this employee" });
          }
        } else {
          ...
          if (!teamIds.has(employeeId)) {
            return res.status(403).json({ error: "You can only send praise for your direct team members" });
          }
        }
      }

      const actorRows = (await db.execute(sql`
        SELECT first_name, last_name FROM admin_users WHERE id = ${actorId} LIMIT 1
      `)).rows as any[];
      const actor = actorRows[0] as any;
      const actorName = actor ? `${actor.first_name} ${actor.last_name}` : "Your manager";

      await storage.createNotification({
        userId: employeeId,
        type: "praise",
        title: "You've been praised!",
        message: `${actorName}: ${praiseNote.trim()}`,
        isRead: false,
        metadata: { fromManagerId: actorId, praiseType: "overtime_recognition" },
      });

      if (notificationId) {
        await db.execute(sql`UPDATE notifications SET is_read = true WHERE id = ${notificationId}`);
      }

      await storage.createAuditLog({
        actorId,
        targetId: employeeId,
        action: "overtime_praise_sent",
        changes: { praiseNote: praiseNote.trim(), notificationId },
      }).catch(console.error);
      ...
      if (flags.email_enabled) {
        try {
          const empRows = (await db.execute(sql`
            SELECT email, first_name FROM admin_users WHERE id = ${employeeId} LIMIT 1
          `)).rows as any[];
          if (empRows.length > 0) {
            const emp = empRows[0] as any;
            if (emp.email) {
              const { sendOvertimePraiseEmail } = await import("./email");
              await sendOvertimePraiseEmail({
                to: emp.email,
                employeeFirstName: emp.first_name,
                managerName: actorName,
                praiseNote: praiseNote.trim(),
              });
            }
          }
        } catch (emailErr) {
          console.error("[praise] Email dispatch failed (non-fatal):", emailErr);
        }
      }

      res.json({ success: true });
    } catch (err) {
      console.error("[praise] send error:", err);
      res.status(500).json({ error: "Failed to send praise" });
    }
  });
