...
**Backdated corrections must not downgrade the live figure:** a correction effective for a *past* month (e.g. from the payroll-preview "correct base salary" tool) must only write `admin_users.salary` when it is the latest-effective applied change. If a later-effective raise already exists, record it in the ledger with `apply:false` (ledger-only) — the target month's slip resolves it by effective date, but the current live salary stays put. Gate on `getSalaryChangesByEmployee` and compare effective dates before deciding `apply`. **Why:** `recordSalaryChange(apply:true)` overwrites the live salary whenever effectiveDate ≤ today, so a naive backdated apply silently reverts a current raise.
